...
interface ReactionGameLogicProps {
  playerName: string;
...
export default function ReactionGameLogic({ 
  playerName, 
  setPlayerName, 
  results, 
  setResults,
  onResultSaved 
}: ReactionGameLogicProps) {
    ...
    <div className="space-y-6">
      <Card className="p-6 border-2 border-[#E5E7EB]">
        <div className="flex items-center gap-3 mb-4">
          <Icon name="Zap" className="text-[#EF4444]" size={28} />
          <h3 className="text-2xl font-bold text-[#1F2937]">Тест реакции</h3>
        </div>
        <p className="text-[#6B7280] mb-6">
          Дождись зелёного цвета, затем нажми как можно быстрее!
        </p>
        ...
        <div className="mb-6">
          ...
          <Input
            ...
            className="max-w-md border-2 border-[#E5E7EB] focus:border-[#3B82F6] text-lg"
          />
        </div>

        <Card className="p-8 bg-[#F9FAFB] flex items-center justify-center border-2 border-[#E5E7EB]">
          <Button
      ...
      {showSpecialMessage && gameState === 'clicked' && specialMessageText && (
        <Card className="p-6 bg-[#FEF3C7] border-2 border-[#F59E0B]">
          <p className="text-center text-lg font-semibold text-[#92400E]">
            {specialMessageText}
          </p>
        </Card>
      )}

      {showSpecialMessage && gameState === 'timeout' && (
        <Card className="p-6 bg-[#FEE2E2] border-2 border-[#EF4444]">
          <div className="flex flex-col items-center gap-4">
            ...
            />
            <p className="text-center text-xl font-bold text-[#991B1B]">
              {getTimeoutMessage()}
            </p>
          </div>
        </Card>
